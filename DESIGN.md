...
This document describes the implemented MVP and explicitly identified future work. Phases 1 through 5 implement configuration, database models and migrations, health checks, durable submission, status queries, independent delivery, bounded retries, attempt records, a mock provider, expired-lease recovery, and manual replay. Crash, shutdown, and database connection recovery are verified with real PostgreSQL and local processes. CI configuration and local Compose demonstration verification are complete, and the user has confirmed that CI passed on GitHub. It follows the original assignment discussed with the user. See [PLAN.md](PLAN.md) for progress.
...
Specific libraries, API fields, and runtime parameters below are implemented defaults selected during development. Future changes should record their rationale here.
...
Unit tests cover validation, content normalization, retry classification, backoff, and Retry-After. Real PostgreSQL integration tests cover concurrent submission deduplication, claiming, lease expiry, stale-token write rejection, crashes during the final attempt, and replay races. A mock provider verifies header and body forwarding, timeouts, redirect handling, and no further delivery after success. Independent local process tests verify continued processing after worker restart and possible duplicate delivery when results were not persisted. The isolated Compose verification now also checks delivery/replay, worker restart and SIGKILL recovery, PostgreSQL restart, and retained data after container recreation. Earlier image download delays were handled by seeding BuildKit with the existing project uv cache, after which the final image and cached rebuilds passed.
...
## 11. Final Build and CI Verification

The Dockerfile separates locked third-party dependency installation from application installation. Both uv operations use a BuildKit cache mount, so source and documentation changes do not force dependency downloads. The runtime still uses Python 3.14, uv 0.12.5, non-editable application installation, and the existing non-root application user. No application schema, retry contract, or dependency version changed in phase 5. `MOCK_PROVIDER_PORT` makes the demonstration port configurable while retaining the default of 8002.

`scripts/verify_compose.py` creates a unique project, dynamically selected localhost ports, and a fresh PostgreSQL volume. It builds and starts the real images, runs the delivery/replay demonstration, restarts the worker with queued tasks, kills it after a provider receives a request, and checks two provider calls plus unknown-outcome/succeeded database history. It also restarts PostgreSQL, verifies readiness 503 during the outage and successful processing after recovery, and recreates containers with the volume retained. Cleanup removes only resources belonging to that generated project. Short timing settings speed verification without changing the state-machine rules.

The GitHub Actions workflow uses pinned official checkout and setup-uv actions, read-only permissions, a PostgreSQL 17 service, locked installation, Ruff, all tests, and the isolated Compose verification. Push, pull-request, and manual triggers are configured; there is no automatic deployment. All corresponding commands were verified locally and the YAML was parsed and checked. The user has since confirmed that CI passed on GitHub; hosted logs and test counts were not independently inspected during this documentation update. References for the selected build/CI patterns are the official uv [Docker integration](https://docs.astral.sh/uv/guides/integration/docker/) and [GitHub Actions integration](https://docs.astral.sh/uv/guides/integration/github/) guides.

The phase 5 local checks were 106 passing tests, lint and formatting checks, migration/model consistency as part of the database suite, and full Compose verification. Remaining limits are the trusted-internal access boundary, possible duplicate delivery, bounded automatic retries, in-memory mock state, and deferred production operations described above. Two upstream TestClient deprecation warnings remain. Successful image verification used a prepopulated dependency cache after slow host/container downloads; a full uncached network download was not successfully repeated.
